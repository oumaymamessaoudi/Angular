    ...
    .subscribe(async messages => {
      ...
      this.messages = await Promise.all(messages.map(async message => {
        return {
          ...
          textContent: message.textContent ? await this.decryptMessage(message.textContent) || '' : '', // Decrypt the message asynchronously
...
saveEditedMessage(message: ChatMessage, updatedText: string): void {
  if (message.id !== undefined) {
    const encryptedText = this.encryptMessage(updatedText); // Encrypt the updated text

    // Call the chat service to update the message with the encrypted text
    const saveEditedMessageSubscription = this.chatService.updateMessage(message.id, encryptedText)
      .subscribe(
        response => {
          ...
          // Decrypt the updated text after the message is successfully updated
          this.decryptMessage(encryptedText)
            .then(decryptedText => {
